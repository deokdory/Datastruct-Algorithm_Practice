 ...
 public:
  ...
  struct DoublyLinkedListIterator {
   ...
   public:
    ...
    friend bool operator==(DoublyLinkedListIterator left,
                           DoublyLinkedListIterator right) {
  ...
  DoublyLinkedList(const DoublyLinkedList &other)
      : head(nullptr), tail(nullptr), playing(nullptr) {
    if (other.head) {
  ...
  DoublyLinkedList(const std::initializer_list<std::string> &ilist)
      : head(nullptr), tail(nullptr), playing(nullptr) {
    auto it = ilist.begin();
...
struct Playlist {
 ...
 public:
  Playlist(DoublyLinkedList _list) { list = new DoublyLinkedList(); }
  Playlist(const std::initializer_list<std::string> &ilist) {
    list = new DoublyLinkedList(ilist);
  }

  void new_song(std::string name) { list->push_back(name); }

  void play() {
    if (list->head) {
      if (list->playing == nullptr) list->playing = list->head;
    }
  ...
  void remove(std::string name) { list->remove(name); }
};
...
4. 음악을 순환적으로 재생할 수 있어야 합니다(이 기능의 구현은 노래 제목 출력으로
대신합니다)

개발 순서
1. 먼저 원형 데이터 표현을 지원하는 기본 구조를 설계합니다.
2. 앞에서 만든 구조에 데이터 삽입 및 삭제 기능을 구현합니다.
3. 사용자 정의 반복문을 작성합니다. 범위 기반 for 반복문문을 사용하여 모든
원소에 접근할 수 있도록 반복자를 설계해야 합니다. 따라서 자료구조는 원형이지만
begin()과 end()함수는 각각 다른 주소를 반환해야 합니다.
4. 기본 컨테이너를 만든 후, 이 컨테이너를 기반으로 재생 목록에 여러 음악을
저장하고 next(), previous(), printfall(), insert(), remove() 같은 함수를
작성합니다.
*/